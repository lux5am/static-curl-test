...
install_cross_compile() {
    ...
    export CC="${DIR}/${SOURCE_DIR}/bin/${SOURCE_DIR}-cc" \
           ...
           CFLAGS="-O3 -Wno-error=unknown-pragmas -Wno-error=sign-compare -Wno-error=cast-align -Wno-maybe-uninitialized -Wno-error=null-dereference -fPIC" \
...
compile_tls() {
    ...
    local openssl_pic_flags=""
    case "${ARCH}" in
        aarch64)
            # For aarch64, we need to either:
            # 1. Use no-asm (recommended for musl)
            # 2. Or use special compiler flags
            if [ "${LIBC}" = "musl" ]; then
                openssl_pic_flags="no-asm"
                echo "aarch64 with musl detected, using no-asm for PIC compatibility"
            else
                openssl_pic_flags="-fPIC -DOPENSSL_USE_BUILD_DATE"
            fi
            ;;
        *)
            openssl_pic_flags="-fPIC"
            ;;
    esac
...
compile_curl() {
    ...
    if ! find "${PREFIX}/lib" -type f -name "libcurl.so*" | grep -q . ; then
        echo "Compiling libcurl, Arch: ${ARCH}" | tee "${RELEASE_DIR}/running"
        
        # Use pkg-config to get ALL dependencies correctly
        export PKG_CONFIG_LIBDIR="${PREFIX}/lib/pkgconfig:${PREFIX}/lib64/pkgconfig"
        
        # Get all static dependencies
        LIBS=$(pkg-config --libs --static libcurl 2>/dev/null || echo "")
        
        if [ -z "${LIBS}" ]; then
            echo "WARNING: pkg-config failed, using manual linking"
            # Fallback to manual linking (but with -fPIC already set globally)
            LIBS="-lssl -lcrypto -lnghttp2 -lnghttp3 -lngtcp2 -lssh2 -lz -lzstd -lbrotlidec -lbrotlicommon -lidn2 -lunistring -lpsl -lcares -lpthread -ldl -lrt -lc"
        fi
        
        echo "Linking with: ${LIBS}"
        
        libcurl_name="libcurl.so.4.8.0"
        ${CC} -shared -fPIC \
            -L${PREFIX}/lib -L${PREFIX}/lib64 \
            -Wl,--whole-archive ${PREFIX}/lib/libcurl.a \
            -Wl,--no-whole-archive \
            ${LIBS} \
            -Wl,-Bdynamic \
            -o ${PREFIX}/lib/${libcurl_name}
    fi
